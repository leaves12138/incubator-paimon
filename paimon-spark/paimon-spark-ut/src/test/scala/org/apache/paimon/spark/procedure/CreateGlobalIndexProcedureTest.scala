...
class CreateGlobalIndexProcedureTest extends PaimonSparkTestBase with StreamTest {
  ...
  test("create bitmap global index") {
    withTable("T") {
      ...
      assert(output.getInt(0) == 1)
      assert(output.getInt(1) > 0)
      assert(output.getInt(2) > 0)
      assert(output.getLong(3) == 100L)
  ...
  test("global index requires row tracking") {
    withTable("T") {
      spark.sql("CREATE TABLE T (id INT, name STRING) TBLPROPERTIES ('bucket' = '-1')")
      spark.sql("INSERT INTO T VALUES (1, 'a')")
      val exception = intercept[Exception] {
        spark.sql("CALL sys.create_global_index(table => 'test.T', column => 'name')")
      }
      assert(exception.getMessage.contains("row-tracking"))
    }
  }
}
