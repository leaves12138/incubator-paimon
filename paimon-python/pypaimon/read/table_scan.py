...
class TableScan:
    ...
    def __init__(self, table, predicate: Optional[Predicate], limit: Optional[int],
        ...
        self.dataEvolution = self.table.options.get('data-evolution.enabled', 'false').lower() == 'true'
    ...
    def _create_data_evolution_splits(self, file_entries: List[ManifestEntry]) -> List['Split']:
        ...
        for key, file_entries in partitioned_files.items():
            ...
            packed_files: List[List[List[DataFileMeta]]] = self._pack_for_ordered(split_by_row_id, weight_func, self.target_split_size)
