...
from pypaimon.read.reader.data_evolution_file_reader import DataEvolutionFileReader
...
class DataEvolutionSplitRead(SplitRead):
    ...
    def create_reader(self) -> RecordReader:
        ...
        return ConcatRecordReader(suppliers)
    ...
    def _create_union_reader(self, need_merge_files: List[DataFileMeta]) -> RecordReader:
        ...
        for i, bunch in enumerate(fields_files):
            ...
            if not read_fields:
                ...
                if len(bunch.files()) == 1:
                    ...
                    suppliers = [
                        lambda f=file: self._create_file_reader(f) 
        ...
        return DataEvolutionFileReader(row_offsets, field_offsets, file_record_readers)
