...
public class FileIndexProcedureITCase extends CatalogITCaseBase {
    ...
    public void testFileIndexProcedureAddIndex() throws Exception {
        ...
        sql("CALL sys.index('default.T', '', 'file-index.bloom-filter.columns=''k,v''')");
        ...
        for (ManifestEntry entry : list) {
            ...
            try (FileIndexFormat.Reader reader =
                ...
                for (FileIndexReader fileIndexReader : readerSetK) {
                    Assertions.assertThat(predicateK.visit(fileIndexReader)).isTrue();
                }

                predicateK = new PredicateBuilder(table.rowType()).equal(0, 4);
                for (FileIndexReader fileIndexReader : readerSetK) {
                    Assertions.assertThat(predicateK.visit(fileIndexReader)).isFalse();
                }
                ...
                for (FileIndexReader fileIndexReader : readerSetV) {
                    Assertions.assertThat(predicateV.visit(fileIndexReader)).isTrue();
                }
                ...
                for (FileIndexReader fileIndexReader : readerSetV) {
                    Assertions.assertThat(predicateV.visit(fileIndexReader)).isFalse();
                }
            }
        }
    }
    ...
    public void testFileIndexProcedureSchemaEvolution() throws Exception {
        ...
        tEnv.getConfig().set(TableConfigOptions.TABLE_DML_SYNC, true);
        sql("CALL sys.index('default.T', '', 'file-index.bloom-filter.columns=''order_id,v''')");
    ...
    public void testFileIndexProcedureDropIndex() throws Exception {
        ...
        tEnv.getConfig().set(TableConfigOptions.TABLE_DML_SYNC, true);
        sql("CALL sys.index('default.T', '', 'file-index.bloom-filter.columns=''k''')");
