...
public class InternalRowToBytesVisitor implements DataTypeVisitor<BiFunction<DataGetters, Integer, byte[]>> {
    ...
    public BiFunction<DataGetters, Integer, byte[]> visit(DoubleType doubleType) {
        return (row, index) -> {
            if (row.isNullAt(index)) {
                ...
                long x = Double.doubleToLongBits(row.getDouble(index));
                return new byte[] {
                        (byte) (x & 0xff),
                        (byte) (x >> 8 & 0xff),
                        (byte) (x >> 16 & 0xff),
    ...
    public BiFunction<DataGetters, Integer, byte[]> visit(TimestampType timestampType) {
        return (row, index) -> {
            if (row.isNullAt(index)) {
                ...
                long x = row.getTimestamp(index, timestampType.getPrecision()).getMillisecond();
                return new byte[] {
                        (byte) (x & 0xff),
                        (byte) (x >> 8 & 0xff),
                        (byte) (x >> 16 & 0xff),
